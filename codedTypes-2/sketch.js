...
function setup() {
    ...
    // Spread slider (controls clustering spread)
    let spreadSlider = createInput((canvasSize / 3).toString(), 'number');
    spreadSlider.attribute('min', '100');
    spreadSlider.attribute('max', canvasSize.toString());
    spreadSlider.attribute('step', '10');
    spreadSlider.parent(uiParent);
    spreadSlider.input(triggerRedraw);
    window.spreadSlider = spreadSlider; // expose globally
...
function drawWord(wordRaw) {
    ...
    let maxSpread = spreadSlider ? float(spreadSlider.value()) : canvasSize / 3; // Controls clustering spread

    let blurShapes = [];
    let sharpShapes = [];

    let shuffledColors = shuffle(colors);
    let totalShapes = min(word.length, 12);
    for (let i = 0; i < totalShapes; i++) {
        let x = centerX + randomGaussian(0, maxSpread / 2);
        let y = centerY + randomGaussian(0, maxSpread / 2);

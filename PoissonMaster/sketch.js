...
function preload() {
    img = loadImage('assets/love.png'); // make sure you have an image file there
...
function draw() {
  ...
  function drawBoostLayer() {
    ...
    for (let c of boostedCircles) {
      boostLayer.rectMode(CENTER);
      boostLayer.rect(c.x, c.y, c.size, c.size);
    }
  }
  ...
  function exportBaseSVG() {
    ...
    for (let c of circles) {
      if (!boostedCircles.includes(c) && c.size > 0.75) { // ✨ skip super small dots
        svgContent += `<rect x="${c.x - c.size / 2}" y="${c.y - c.size / 2}" width="${c.size}" height="${c.size}" fill="black" />\n`;
      }
    }
  ...
  function exportBoostedSVG() {
    ...
    for (let c of boostedCircles) {
      if (c.size > 0.75) {
        svgContent += `<rect x="${c.x - c.size / 2}" y="${c.y - c.size / 2}" width="${c.size}" height="${c.size}" fill="red" />\n`; 
      svgContent += `<rect x="${c.x - c.size / 2}" y="${c.y - c.size / 2}" width="${c.size}" height="${c.size}" fill="red" />`;
    }
  }
